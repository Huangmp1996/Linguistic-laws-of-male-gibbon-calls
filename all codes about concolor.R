#Second check
#concolor
setwd("D:/Gibbon/Sound/Data analyse/Second check/west")
Zdata<-read.csv("M_phrase_west.csv")
...
ggplot(data=file, aes(x=x,y=freq))+
  ...
  theme_bw()+
  theme(panel.grid=element_blank(),axis.text= element_text(size=23),
        axis.title= element_text(size=27))
#note count for each individual
#the size of pdf need to set 8.5*13 inche
file<- ddply(Zdata,.(individual_ID),function(x){length(x$note_name)
  }
  )
library(ggplot2)
...
Zdata<-read.csv("M_phrase_west.csv")
library(ggplot2)
ggplot(data=Zdata, aes(x=note_name,y=note_duration))+
  geom_boxplot()+
...
Mseq_data<- read.csv("M_seq_west2.csv",header=T,sep=",")
...
#zlaw note mean
setwd("D:/Gibbon/Sound/Data analyse/Second check/west") #set work directory
Zdata<-read.csv("M_phrase_west.csv",sep = ",",stringsAsFactors = F)
...
write.table(dat.sound,file = "Z_law_mean_boom.csv",
            row.names = F,sep = ",")
#LMM Z model
library(lme4)
library(lmerTest)
Z.mean <- read.csv("Z_law_mean_boom.csv",stringsAsFactors = F)
#Z.mean<- Z.mean[-which(Z.mean$note_name=='boom'),]#delete boom for its difficulty to distinguish from noise
...
setwd("D:/Gibbon/Sound/Data analyse/Second check/west")
Zdata<-read.csv("M_phrase_west.csv")
#Zdata<- Zdata[-which(Zdata$note_name=='boom'),]
dat.sound <- data.frame()
...
#M law
#M law
Mseq_data<- read.csv("M_seq_west2.csv",header = T,sep = ",",stringsAsFactors = F) #read file
#model selection
library(MuMIn)
...
#M LMM model
Mseq.model <- lmer(note_duration~seqsize+note_position_in_sequence+
...
setwd("D:/Gibbon/Sound/Data analyse/Second check/west") #set work directory
Mseq_data<- read.csv("M_seq_west2.csv",header=T,sep=",",stringsAsFactors = F)
